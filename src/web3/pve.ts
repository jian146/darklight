...
import { I_claimAllApi } from 'src/pages/api/request'
import { getHero, getOldHero, getPve } from '.'
...
export const getPveHeroList = async (provider: Web3Provider, isOld=false): Promise<pve_heroType[]> => {
  ...
  const hero =isOld?getOldHero(provider): getHero(provider)
...
export const newBattle = async (provider: Web3Provider, mapId:number, lv:number, tokenId:string): Promise<ContractReceipt> => {
  const pveProvider = getPve(provider)
  try {
    const res = await pveProvider.newBattle(mapId+1, lv, tokenId, {gasLimit: 1000000})
...
export const claimAllBattleAbi = async (provider: Web3Provider, backData:I_claimAllApi):Promise<null> => {
  const pveProvider = getPve(provider)
  try {
    const res = await pveProvider.claimAllBattle(backData.battleIds, backData.allDlts, backData.allGolds, backData.equips, backData.sig)
    return await res.wait()
  } catch (error) {
    return Promise.reject(error)
  }
