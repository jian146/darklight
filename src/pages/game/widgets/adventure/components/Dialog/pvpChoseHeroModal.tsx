...
const PvpChoseHeroModal: React.FC<I_ChoseHeroModal> = (
  props: I_ChoseHeroModal
) => {
  const { onCancelbtnClick, onOkbtnClick, seletedAdv, heroVersion } = props
 ...
 */
  ...
  const dltPrice=selHeroAttr.hero?.level?(seletedAdv as mapType_price).price['lv'+selHeroAttr.hero?.level]??0:0
    ...
    >
      <Spin spinning={loading}>
        ...
        <p style={{ color: '#B2856C' }} className="text-center text-xl">
          {t(`game.arena.map.${seletedAdv?.title}`)}
        </p>
        {/* 关闭按钮 */}
        <img
        ...
        <div className="flex justify-center pb-4">
          <Button
            ...
            className="mt-5"
            img={btnBg}
            disabled={!selHeroAttr.hero }
            onClick={ async () =>{
              setLoading(true)

              if (!await checkDlt(library, dltPrice)){
              ...
              !!selHeroAttr.hero&&await onOkbtnClick(selHeroAttr.hero, selHeroAttr.hero.level??0)
              setLoading(false)
            }}
          >
            {t('game.arena.matchOpponents')}
          </Button>
        </div>
      </Spin>
    </ChoseHeroModalStyle>
...
export default PvpChoseHeroModal
